...
class MAX30100(object):
    ...
    def read_sensor(self):
        bytes = self.i2c.read_i2c_block_data(I2C_ADDRESS, FIFO_DATA, 8)
        # Add latest values.
        self.buffer_ir.append(bytes[0]<<8 | bytes[0])
        self.buffer_red.append(bytes[2]<<8 | bytes[3])
        
        # Crop our local FIFO buffer to length.
        #print("reading..")
        print("Bytes print from read sensor func")
        print(bytes)
        print(type(bytes))
        x=bytes[0]
    ...
    def get_temperature(self):
        intg = _twos_complement(self.i2c.read_byte_data(I2C_ADDRESS, TEMP_INTG),4)
